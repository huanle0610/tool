...
function GetDianPosList(val) {
    var dianpos = currentInput.value.indexOf('.');
    var tempstr = "";
    while (dianpos > 0) {
        tempstr = tempstr + "," + dianpos;
        dianpos = currentInput.value.indexOf('.', dianpos+1);
    }
    if (tempstr != "") {
...
function SelectAreaText() {
    var pos = getPosition(currentInput);
    if (pos == 0) return;
    var dianposlist = GetDianPosList(currentInput.value);
    if (dianposlist == null) {
        return;
    }
    if (pos <= parseInt(dianposlist[0])) {
        SelectRange(currentInput, 0, parseInt(dianposlist[0]));
    }
    else if (pos > parseInt(dianposlist[0]) && pos <= parseInt(dianposlist[1])) {
        SelectRange(currentInput, parseInt(dianposlist[0]) + 1, parseInt(dianposlist[1]));
    }
    else if (pos > parseInt(dianposlist[1]) && pos < currentInput.value.length) {
        SelectRange(currentInput, parseInt(dianposlist[1]) + 1, currentInput.value.length);
    }
}
function BoxShow(e) {
    var input = e;
...
function InputMouseUp() {
    if (!iswords && currentInput.value != "" && GetSelectText() == '') {
        SelectAreaText();
    }
}
function addInput(e) {
...
function Init() {
    ...
    for (var i = 0; i < tags.length; i++) {
        if (tags[i].getAttribute('url') == 'true') {
            ...
            attach(tags[i], 'mouseup', InputMouseUp);
